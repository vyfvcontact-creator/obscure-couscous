  ...
  async start() {
    ...
    this.sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        console.log('Connected to WhatsApp - Bot ready for section 3/13');
      }
    });
    ...
    this.sock.ev.on('group-participants.update', async (update) => {
      if (update.action === 'add') {
        const welcomeMsg = `مرحباً بك في مجموعة قسم 3/13! 🤖\n\nأنا بوت المساعدة الأكاديمية لمدرسة ابن خلدون الإعدادية خريبكة.\nمتخصص في حل التمارين والمساعدة في المواد الدراسية.\n\nاستخدم !help لمعرفة الأوامر المتاحة. 📚✨`;
